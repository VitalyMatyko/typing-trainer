...
const useTypingLogic = () => {
	...
	const calculateStats = () => {
		...
		const time = Math.floor(elapsedTime * 1000) / 1000;
	...
	const getDailyGoalTimeValue = () => getGoalValue(goalData.timeTyping, goalData.dailyGoal);
		...
		if (speed >= goalData.speedGoal) {
			setInterfaceData((prev) => {
				if (language) {
					...
					if (!charArray || !charArray[currentIndex]) return prev;
